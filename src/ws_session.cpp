...
{
#ifdef ENABLE_SSL
    ws_session::ws_session(server &srv, std::string_view path, asio::ssl::stream<asio::ip::tcp::socket> &&socket) : srv(srv), path(path), endpoint(socket.lowest_layer().remote_endpoint()), socket(std::move(socket)) { LOG_TRACE("WebSocket session created with " << endpoint); }
#else
    ws_session::ws_session(server &srv, std::string_view path, asio::ip::tcp::socket &&socket) : srv(srv), path(path), endpoint(socket.remote_endpoint()), socket(std::move(socket)) { LOG_TRACE("WebSocket session created with " << endpoint); }
#endif
    ...
    void ws_session::on_read(const std::error_code &ec, std::size_t)
    { // read the first two bytes of the message (opcode and length)
        ...
        if (length == 126)
            asio::async_read(socket, msg->buffer, asio::transfer_exactly(2), [self = shared_from_this()](const std::error_code &, std::size_t)
                             { char buf[2];
            ...
            asio::async_read(socket, msg->buffer, asio::transfer_exactly(8), [self = shared_from_this()](const std::error_code &, std::size_t)
                             { char buf[8];
                                      std::istream is(&self->msg->buffer);
                                      is.read(buf, 8);
                                      size_t length = 0;
    ...
    void ws_session::on_write(const std::error_code &ec, std::size_t)
    {
        if (ec)
        {
            LOG_ERR(ec.message());
